...
class OperadorRepository extends BaseRepository
{
	...
	{
		$english=isset($inputs['nombre_servicio_eng'])?$inputs['nombre_servicio_eng']:"";
		
		$usuarioServicio->nombre_servicio = $inputs['nombre_servicio'];
		$usuarioServicio->detalle_servicio = $inputs['detalle_servicio'];
		$usuarioServicio->nombre_servicio_eng = $english;
		$usuarioServicio->key_words = $inputs['key_words'];
	...
	public function updateUsuarioServiciosTour($inputs)
	{
		

			$english=isset($inputs['title_esp'])?$inputs['title_esp']:"";
			$keywords=isset($inputs['title_eng'])?$inputs['title_eng']:"";
			$keywords_eng=isset($inputs['url_esp'])?$inputs['url_esp']:"";
			$meta_description_esp=isset($inputs['url_eng'])?$inputs['url_eng']:"";
			$meta_description_eng=isset($inputs['h1_esp'])?$inputs['h1_esp']:"";
			$h1_eng=isset($inputs['h1_eng'])?$inputs['h1_eng']:"";
			$h1_esp=isset($inputs['meta_descrip_eng'])?$inputs['meta_descrip_eng']:"";
			$face_image_mobile=isset($inputs['meta_descrip_eng'])?$inputs['meta_descrip_eng']:"";
			$face_image_desk=isset($inputs['meta_descrip_esp'])?$inputs['meta_descrip_esp']:"";
			$url_esp=isset($inputs['faceb_img_desk'])?$inputs['faceb_img_desk']:"";
			$url_eng=isset($inputs['faceb_img_mobile'])?$inputs['faceb_img_mobile']:"";
		
	

			$updateData = DB::table('booking_abcalendar_agrupamiento')
			->where('id',$inputs['id_agrupamiento'])
			->update(['title_esp' => $inputs['title_esp'],		
				'title_eng' => $inputs['title_eng'],		
					'url_esp' => $inputs['url_esp'],		
						'url_eng' => $inputs['url_eng'],		
							'h1_esp' => $inputs['h1_esp'],			
							'h1_eng' => $inputs['h1_eng'],		
								'meta_descrip_eng' => $inputs['meta_descrip_eng'],
											'meta_descrip_esp' => $inputs['meta_descrip_esp'],
											'faceb_img_desk' => $inputs['faceb_img_desk'],		
												'faceb_img_mobile' => $inputs['faceb_img_mobile'],'title_esp' => $inputs['title_esp']]);
...
return $updateData;
	...
	public function updateUsuarioServicios($usuarioServicio, $inputs)
	{
			...
			$english=isset($inputs['nombre_servicio_eng'])?$inputs['nombre_servicio_eng']:"";
			$keywords=isset($inputs['key_words'])?$inputs['key_words']:"";
			$keywords_eng=isset($inputs['key_words_eng'])?$inputs['key_words_eng']:"";

			$meta_description_esp=isset($inputs['meta_description_esp'])?$inputs['meta_description_esp']:"";
			$meta_description_eng=isset($inputs['meta_description_eng'])?$inputs['meta_description_eng']:"";
			$h1_eng=isset($inputs['h1_eng'])?$inputs['h1_eng']:"";
			$h1_esp=isset($inputs['h1_esp'])?$inputs['h1_esp']:"";

			$face_image_mobile=isset($inputs['face_image_mobile'])?$inputs['face_image_mobile']:"";
			$face_image_desk=isset($inputs['face_image_desk'])?$inputs['face_image_desk']:"";
			$url_esp=isset($inputs['url_esp'])?$inputs['url_esp']:"";
			$url_eng=isset($inputs['url_eng'])?$inputs['url_eng']:"";
		
		if (isset($inputs['id_catalogo_eventos']))
        {
		return  $usuarioServicio::where('id',$inputs['id_usuario_servicio'])
									->update(['nombre_servicio'=>$inputs['nombre_servicio'],
											...
											'nombre_servicio_eng'=>$english, 
											'key_words'=>$keywords, 
											'key_words_eng'=>$keywords_eng, 
											
											'meta_description_esp'=>$meta_description_esp, 
											'meta_description_eng'=>$meta_description_eng, 
											'h1_eng'=>$h1_eng, 
											'h1_esp'=>$h1_esp, 

											'face_image_mobile'=>$face_image_mobile, 
											'face_image_desk'=>$face_image_desk, 
											'url_esp'=>$url_esp, 
											'url_eng'=>$url_eng, 
...
//											'observaciones'=>$inputs['observaciones'],
											...
											'longitud_servicio'=>$inputs['longitud_servicio'],
                                                                            ...
                                                                            'id_catalogo_eventos' => $inputs['id_catalogo_eventos'],
...
// exit;	
	...
	public function storageUsuarioAgrupamientosTour($inputs)
	{
            
	
			$x=$this->updateUsuarioServiciosTour($inputs);
