...
class ServiciosOperadorRepository extends BaseRepository {
    ...
    public function __construct(Usuario_Servicio $userservicios) {
		...
		$this->usuario_serviciosModel = new Usuario_Servicio();
...
//Entrega el arreglo de Servicios por operador
    ...
    public function getServiciosOperadorUnicos($id_usuario_operador) {
		...
		if($id_usuario_operador ==2||$id_usuario_operador ==19||$id_usuario_operador ==21||$id_usuario_operador ==18)
		{
		return DB::table('usuario_servicios')
  ...
  //**************************************************************************//
    //           DEVUELVE EL Los ultimos OPERADORes creados                     //
    //**************************************************************************//
    public function getLatestServices() {
        
        return $this->usuario_serviciosModel
                        ->with('profile_image')
						->where('usuario_servicios.estado_servicio','=',1)
                        ->select(['usuario_servicios.id','nombre_servicio','detalle_servicio','usuario_servicios.created_at'])
                        ->orderBy('usuario_servicios.created_at','DESC')
                        ->limit(4)
                        ->get();
    }
	
	public function getServiciosSubscriptions($id_usuario_externo) {
        $emailData = DB::table('users')
                    ->join('subscriptions','users.email','=','subscriptions.email')
                    ->join('subscription_type_pivot','subscription_type_pivot.id_subscription','=','subscriptions.id_subscription')
                    ->select(['id_type'])
                    ->where('users.id',$id_usuario_externo)
                    ->get();
