...
function sendMailBooking(token,nombre){
    var obj = {token: token, nombre: nombre};
    var url ='/sendMailBooking';
    $.ajax({
        type: 'POST',
        url: url,
        data: obj,
...
function GetDataAjaxSearchEvents(url,contador) {
    ...
    $.ajax({
        ...
        success: function (data) {
            console.log('Exitoso');
            console.log(data);
            $('.SearchTotalPartial').html(data.topEvent)
            $(".woocommerce").LoadingOverlay("hide", true);           
        },
        error: function (data) {
            $(".woocommerce").LoadingOverlay("hide", true);
            console.log('ERROR');
...
function AjaxContainerRegistroLoadFTours($formulario,$load) {
    $("."+$load).LoadingOverlay("show");
    var $form = $('#' + $formulario),
            data = $form.serialize(),
            url = $form.attr("action");
...
function AjaxCronReviewsTours(url) {
    console.log('Entro a la funcion URL', url);
    $(".woocommerce").LoadingOverlay("show");
    $.ajax({
        type: 'GET',
        url: url,
        ...
        error: function (data) {          
            $(".woocommerce").LoadingOverlay("hide", true); 
            alert('Cron Review con errores al ejecutarlo');  
            var errors = data.responseJSON;
            if (errors) {
                $.each(errors, function (i) {
                    console.log(errors[i]);
